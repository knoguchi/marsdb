...
この階層は単なる書き方の好みではなく、redb の制約に対応するためのものです。redb は
...
パスでも役立つ点が 1 つあります。クリーンアップに必要なのは
...
不変条件違反は明示するという方針に対し、この非対称性は意図的です。
カウンタはプランナ統計であり、誤った値は最適ではない実行計画を生むことは
あっても誤答は決して生まない。だからデータベースを落とすのではなく
誤った推定値にとどめるべきです。不変条件違反をどれだけ厳しく扱うかは、
その不変条件が保護する対象の重要性に応じて決めています。
...
性能は変わりませんでした。そのため doc コメントには、高速化ではなく、
API を扱いやすくし、重複処理を確実に減らすことが採用理由だと明記して
います。

## 整合性チェッカーは不変条件の仕様書

`check_integrity` を上のミューテーション群と背中合わせに読むのが、
書き込み処理の契約を理解する近道です。チェッカーは、不変条件を
実行可能な仕様として表現しているからです。
