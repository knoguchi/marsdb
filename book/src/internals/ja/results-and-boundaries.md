...
バルク結果のために ABI は**バイナリバッチレーン**を提供します:
1 回の呼び出しが結果全体を、コンパクトな自己記述バッファ —
インターンされた列名とプロパティ名、varint 整数 — として返し、
バインディングが自分の言語でデコードします。値ごとではなく
*クエリ*ごとに 1 回の境界越え: 呼び出しごとの FFI 税(マーシャリング、
エラー検査、ランタイムによってはロック取得)は一度だけ払われます。
ストリーミングコールバックレーンが逆の形 — 有界メモリでの無制限
エクスポート — をカバーし、エグゼキュータのストリーミングパスを
通じてコールバックごとに 1 行を押し出します。
...
同じバッチレーン戦略を、そして同じ実行制限とトランザクションの表面を
公開します。

## Arrow: 列指向の境界

分析系のコンシューマ — データフレーム、列指向計算 — にとって、行の
結果は誤った形であり、値ごとの変換は誤ったコストです。MarsDB の
...
バッチレーンはクエリあたりバインディング側で約 120 万回のアロケー
ション(83 MB)を行い、Arrow レーンは約 900 回(83 KB)です — 壁時計
時間は同等。どちらもエンジンの実行が支配的だからですが、アロケータと
