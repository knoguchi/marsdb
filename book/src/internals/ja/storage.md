...
テーブルは id で会話します。これは古典的な「空間 対 間接参照」の
取引です — レコードは繰り返される文字列の代わりに固定 4 バイトを
格納し、比較は整数比較になります — 代償は境界での変換 1 回です。
...
*コストを見積もる*ためだけに使われます — 誤った値は準最適なプランを
...
テーブルを先回りして開き — つまり作成 — します。redb はテーブルを
