...
marsdb-query     Cypher テキスト -> AST -> 論理実行計画 -> エグゼキュータ
...
flowchart BT
    ...
    query["marsdb-query — Cypher → AST → 実行計画 → エグゼキュータ"]
...
実績あるエンジンを土台にすることで、エンコーディング、実行計画の作成、
...
「グラフをどのように探索するか」を表す構造になっているのは意図的です。
それにより、プランナはアクセスパスの置き換え(ラベルスキャンを
インデックスシークに、アンカー付き展開をエッジ一括スキャンに)を、
エグゼキュータが実行計画の生成過程を意識することなく行えます。
...
5. **実行計画の作成。** パターンを論理演算子ツリーへ変換し、次に
   ストレージを見ながら書き換えられます: 宣言済みインデックスが
   あればラベルスキャン上のフィルタがインデックスシークになり、
   コスト統計に基づいて開始点が反転されることがあり、述語と LIMIT が
   スキャンへ向けて押し下げられます。実行計画は実行のたびに、
   文のトランザクションの中で作成されます。そのため実行計画には、まさに
   このスナップショットが見られるインデックスと統計を反映します。

6. **評価。** エグゼキュータは演算子ツリーを再帰的に評価し、変数をノード・
...
エラーはトランザクションが存在する前に起き、実行計画の作成は対象の
